...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim) {
	if (htim == distanceSensor.echoTimer) {
		val[valIdx++] = HCSR04_Read(&distanceSensor);
		if (valIdx >= 10) {
			uint32_t avg = 0;
			for (int i = 0; i < 10; ++i) {
				avg += val[i];
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  HAL_GPIO_WritePin(GPIOB, LD1_Pin|LD3_Pin|Test_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : LD1_Pin LD3_Pin Test_Pin */
  GPIO_InitStruct.Pin = LD1_Pin|LD3_Pin|Test_Pin;
